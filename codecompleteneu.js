/* =========================== KREISFÖRMIGER SLIDER =========================== */
function initCircularPopupSlider() {
  ...
  window.initSlider = function () {
    ...
    if (popup.__sliderEventsBound) return;
    popup.__sliderEventsBound = true;
    ...
    if (window.innerWidth < 768) {
      ...
      popup.addEventListener(
        ...
        (e) => {
          ...
          targetProgress += deltaY * SPEED * 2.8;
          targetProgress = gsap.utils.clamp(0, 1, targetProgress);
    ...
    for (let i = 0; i < config.imageCount; i++) {
      ...
      card.className = "card";
      card.dataset.title =
        window.collection[i % window.collection.length].title;

      const img = document.createElement("img");
      img.src = window.collection[i % window.collection.length].img;
    ...
    function showTitle(card) {
      ...
      const data = window.collection.find(
        (c) => c.title === card.dataset.title
      );
    ...
    document.addEventListener("mousemove", (e) => {
      ...
      parallaxState.targetY = ((e.clientX - cx) / cx) * 15 * hoverStrength;
      parallaxState.targetX = -((e.clientY - cy) / cy) * 15 * hoverStrength;
      ...
      cards.forEach((card, i) => {
        ...
        if (d < config.sensitivity) {
          const f =
            Math.max(0, 1 - d / config.effectFalloff) * hoverStrength;
    ...
    function animate() {
      ...
      if (window.innerWidth < 768) {
        const titleVisibleMobile = p > 0.22;
  ...
  if (!window.bindPopup4SliderObserver) {
    window.bindPopup4SliderObserver = function () {
      const popup = document.querySelector(".popup-wrapper-4");
      if (!popup) return;
...
/* =========================== POPUP OBSERVER =========================== */
function bindPopup4SliderObserver() {
  const popup = document.querySelector(".popup-wrapper-4");
  if (!popup) return;
  ...
  if (popup.__sliderObserverBound) return;
  popup.__sliderObserverBound = true;
...
function initBarbaAndTransitions() {
  ...
  barba.hooks.after((data) => {
    ...
    bindPopup4SliderObserver();
    initCircularPopupSlider();
  });
}
